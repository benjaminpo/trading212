...
export function aggregateAccountStats(
  accountResults: Array<{ data: MockAccountData }>,
) {
  ...
  for (const result of accountResults) {
    ...
    const accountCurrentValue =
      data.portfolio?.reduce(
        (sum: number, pos: any) => sum + pos.quantity * pos.currentPrice,
        0,
      ) || 0;
  ...
  const todayPnLPercent =
    totalInvestedValue > 0 ? (todayPnL / totalInvestedValue) * 100 : 0;
      ...
      const accountResults = [
        {
          data: {
            ...
            portfolio: [{ quantity: 5, currentPrice: 100 }],
          }, // No cash property
        },
        {
          data: {
