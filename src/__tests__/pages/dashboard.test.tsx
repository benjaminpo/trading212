...
jest.mock('next-auth/react', () => ({
  useSession: jest.fn(() => ({ data: { user: { name: 'Test User' } }, status: 'authenticated' })),
}))
...
describe('Dashboard page behaviors', () => {
      ...
      if (url.includes('/api/trading212/optimized/account?accountId=a1')) {
        return {
          ...
          json: async () => ({
            connected: true,
            error: null,
            stats: { totalPnL: 1000, totalPnLPercent: 10, todayPnL: 50, todayPnLPercent: 0.5, activePositions: 3, trailStopOrders: 1 },
            account: { id: 'a1', name: 'Acc1', isPractice: false, currency: 'USD', cash: 5000 },
  ...
  it('reloads data on window focus', async () => {
    ...
    await waitFor(() => expect(callCounts.accounts + callCounts.ai + callCounts.account).toBeGreaterThan(0))
    ...
    await waitFor(() => {
      ...
      expect(callCounts.accounts + callCounts.ai + callCounts.account).toBeGreaterThan(1)
    })
  })
  ...
  it('switches to single-account data when account is selected', async () => {
    ...
    const trigger = await screen.findByRole('button', { name: /all accounts/i }, { timeout: 4000 })
    await act(async () => { trigger.click() })
    const accOption = await screen.findByText('Acc1', undefined, { timeout: 4000 })
    await act(async () => { accOption.click() })
