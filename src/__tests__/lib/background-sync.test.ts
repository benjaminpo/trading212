import { BackgroundSyncService, backgroundSyncService } from '../../lib/background-sync'
...
describe('BackgroundSyncService', () => {
  ...
  describe('start()', () => {
    ...
    it('should not start if already running', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('🔄 Background sync is already running')
  ...
  describe('stop()', () => {
    it('should stop the service successfully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('⏹️ Background sync service stopped')
  ...
  describe('runSync()', () => {
    it('should sync users successfully', async () => {
      ...
      expect(optimizedTrading212Service.backgroundSync).toHaveBeenCalledWith('user1', [
        { id: 'acc1', apiKey: 'key1', isPractice: false, name: 'Account 1' },
        { id: 'acc2', apiKey: 'key2', isPractice: true, name: 'Account 2' }
      ])
    })
    ...
    it('should handle user sync errors', async () => {
      ...
      const mockUsers = [
        {
          id: 'user1',
          trading212Accounts: [{ id: 'acc1', apiKey: 'key1', isPractice: false, name: 'Account 1' }]
        }
      ]
      
      prisma.user.findMany.mockResolvedValue(mockUsers)
      optimizedTrading212Service.backgroundSync.mockRejectedValue(new Error('Sync error'))
      ...
      expect(consoleSpy).toHaveBeenCalledWith('❌ Background sync failed for user user1:', expect.any(Error))
    ...
    it('should handle database errors', async () => {
      ...
      expect(stats.accountsProcessed).toBe(0)
      expect(stats.errors).toBe(1)
      expect(consoleSpy).toHaveBeenCalledWith('❌ Background sync error:', expect.any(Error))
  ...
  describe('syncUser()', () => {
    ...
    it('should handle sync errors', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('❌ User sync failed for user1:', expect.any(Error))
  ...
  describe('clearOldCache()', () => {
    it('should clear old cache successfully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('🗑️ Cleared 5 old AI recommendations')
    ...
    it('should handle cache cleanup errors', async () => {
      ...
      prisma.aIRecommendation.deleteMany.mockRejectedValue(new Error('Delete error'))

      const result = await syncService.clearOldCache()

      expect(result).toBe(0)
      expect(consoleSpy).toHaveBeenCalledWith('❌ Cache cleanup error:', expect.any(Error))
  ...
  describe('healthCheck()', () => {
    it('should return health status when running', async () => {
      ...
      optimizedTrading212Service.getCacheStats.mockReturnValue({ cache: 'stats' })
      optimizedTrading212Service.getBatchStats.mockReturnValue({ batch: 'stats' })
