...
describe("DailyAnalysisScheduler", () => {
  ...
  describe("start", () => {
    ...
    it("should start scheduler and run analysis immediately", async () => {
      ...
      const _runDailyAnalysisSpy = jest
        .spyOn(scheduler, "runDailyAnalysis")
  ...
  describe("runDailyAnalysis", () => {
    ...
    beforeEach(() => {
      ...
      (aiAnalysisService.analyzeBulkPositions as jest.Mock).mockResolvedValue(
        mockAIRecommendations,
      );
      (mockedPrisma.position.upsert as jest.Mock).mockResolvedValue({}, 10000);
      (mockedPrisma.position.findFirst as jest.Mock).mockResolvedValue(
        { id: "position1" },
        10000,
      );
      ...
      (optimizedTrading212Service.canMakeRequest as jest.Mock).mockReturnValue(
        true,
      );
      (
        optimizedTrading212Service.getAccountData as jest.Mock
      ).mockResolvedValue({
        connected: true,
        error: null,
        account: {
    ...
    it("should handle AI analysis errors", async () => {
      (aiAnalysisService.analyzeBulkPositions as jest.Mock).mockRejectedValue(
        new Error("AI Error"),
      );
  ...
  describe("analyzeUser", () => {
    ...
    it("should analyze specific user", async () => {
      const analyzeUserPositionsSpy = jest
        .spyOn(scheduler as any, "analyzeUserPositions")
        .mockResolvedValue();
      ...
      expect(analyzeUserPositionsSpy).toHaveBeenCalledWith(
        "user1",
        "test-api-key",
        true,
      );
    ...
    it("should throw error if user has no active accounts", async () => {
      ...
      await expect(scheduler.analyzeUser("user1")).rejects.toThrow(
  ...
  describe("analyzeUserPositions", () => {
    ...
    it("should analyze user positions successfully", async () => {
      ...
      await (scheduler as any).analyzeUserPositions(
        "user1",
        "test-api-key",
        true,
      );
