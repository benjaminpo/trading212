...
describe('APICache', () => {
  ...
  describe('Cache Operations', () => {
    ...
    it('should support parameters in cache keys', async () => {
      ...
      const notFound = await cache.get(userId, accountId, dataType, differentParams)
  ...
  describe('Cache Invalidation', () => {
    ...
    beforeEach(async () => {
      ...
      await cache.set(userId, accountId1, 'account', { data: 'account1-account' })
    })
  ...
  describe('Memory Management', () => {
    it('should enforce memory limits', async () => {
      ...
      for (let i = 0; i < MAX_ENTRIES + 100; i++) {
        await cache.set(`user${i}`, `account${i}`, 'portfolio', { data: `test${i}` })
      }
  ...
  describe('Database Cache Methods', () => {
    it('should handle database cache get operations', async () => {
      const result = await cache.getFromDatabase('user1', 'account1', 'portfolio')
      expect(result).toBeNull() // Returns null as per current implementation
    })

    it('should handle database cache set operations', async () => {
      await expect(cache.setInDatabase('user1', 'account1', 'portfolio', { data: 'test' }))
  ...
  describe('Error Handling', () => {
    ...
    it('should handle cache set with invalid data', async () => {
      await expect(cache.set('user1', 'account1', 'portfolio', null as any))
        .resolves.not.toThrow()
    })
  })
