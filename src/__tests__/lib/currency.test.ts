...
describe("Currency utilities", () => {
  ...
  describe("formatCurrency with options", () => {
    it("handles showSymbol option", () => {
      expect(formatCurrency(1234.56, "USD", { showSymbol: false })).toBe(
        "1,234.56",
      );
      expect(formatCurrency(1234.56, "USD", { showSymbol: true })).toBe(
        "$1,234.56",
      );
    });
    ...
    it("handles showCode option", () => {
      expect(
        formatCurrency(1234.56, "USD", { showCode: true, showSymbol: false }),
      ).toBe("1,234.56 USD");
      expect(
        formatCurrency(1234.56, "EUR", { showCode: true, showSymbol: false }),
      ).toBe("1.234,56 EUR");
    });
    ...
    it("handles multiple options together", () => {
      expect(
        formatCurrency(1234.56, "USD", { showSymbol: false, showCode: true }),
      ).toBe("1,234.56 USD");
      expect(
        formatCurrency(1000, "USD", { showSymbol: true, compact: true }),
      ).toBe("$1.00K");
    });

    it("handles currencies with symbol after position", () => {
      expect(formatCurrency(1234.56, "CHF")).toBe(
        `1${String.fromCharCode(8217)}234.56 CHF`,
      );
      expect(formatCurrency(1234.56, "SEK")).toBe(
        `1${String.fromCharCode(160)}234,56 kr`,
      );
    });
