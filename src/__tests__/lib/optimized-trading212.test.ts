import {
  ...
  OptimizedAccountData,
} from "@/lib/optimized-trading212";
...
const mockRateLimiter = trading212RateLimiter as jest.Mocked<
...
describe("OptimizedTrading212Service", () => {
  ...
  const mockAccountData = {
    ...
    portfolio: [
      { ticker: "AAPL", quantity: 100, currentPrice: 150, ppl: 1000 },
    ],
    ...
    beforeEach(() => {
      mockApiBatcher.fetchMultiAccountData.mockResolvedValue([
        { accountId: "account1", data: mockAccountData, error: undefined },
        {
          ...
          data: {
            ...mockAccountData,
            stats: { ...mockAccountData.stats, totalPnL: 2000 },
          },
          error: undefined,
        },
      ]);
  ...
  describe("Rate Limiting", () => {
    it("should check if request can be made", () => {
      const canMake = optimizedTrading212Service.canMakeRequest(
        userId,
        accountId,
      );
      expect(mockRateLimiter.canMakeRequest).toHaveBeenCalledWith(
        `trading212-${userId}-${accountId}`,
      );
    ...
    it("should get time until reset", () => {
      ...
      expect(mockRateLimiter.getTimeUntilReset).toHaveBeenCalledWith(
        `trading212-${userId}-${accountId}`,
      );
  ...
  describe("Cache Management", () => {
    it("should invalidate cache", async () => {
      await optimizedTrading212Service.invalidateCache(
        userId,
        accountId,
        "portfolio",
      );

      expect(mockApiCache.invalidate).toHaveBeenCalledWith(
        userId,
        accountId,
        "portfolio",
      );
  ...
  describe("Background Sync", () => {
    ...
    it("should perform background sync when rate limits allow", async () => {
      ...
      expect(mockApiBatcher.fetchMultiAccountData).toHaveBeenCalledWith(
        userId,
        accounts,
        false,
      );
    ...
    it("should skip sync when rate limited", async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining("All accounts are rate limited"),
      );
    });

    it("should handle sync errors gracefully", async () => {
      mockRateLimiter.canMakeRequest.mockReturnValue(true);
      mockApiBatcher.fetchMultiAccountData.mockRejectedValue(
  ...
  describe("Error Handling", () => {
    it("should handle API batcher errors", async () => {
      mockApiCache.get.mockResolvedValue(null);
      mockApiBatcher.fetchAccountData.mockRejectedValue(
        new Error("Batcher error"),
      );
