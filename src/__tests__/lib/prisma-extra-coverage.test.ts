...
describe('prisma extra coverage', () => {
  it('retryDatabaseOperation should throw immediately on non-retryable error', async () => {
    const op = jest.fn().mockRejectedValueOnce(Object.assign(new Error('validation failed'), { code: 'P2002' }))
    await expect(retryDatabaseOperation(op, 1)).rejects.toThrow('validation failed')
  })
