...
describe("AIAnalysisService", () => {
  ...
  describe("analyzePosition", () => {
    ...
    it("should handle different risk profiles in rule-based analysis", async () => {
      ...
      const moderateResult = await aiAnalysisService.analyzePosition(
        mockPosition,
        mockMarketData,
        "MODERATE",
      );
  ...
  describe("analyzeBulkPositions", () => {
    it("should analyze multiple positions", async () => {
      ...
      const results = await aiAnalysisService.analyzeBulkPositions(
        positions,
        marketData,
        "MODERATE",
      );
  ...
  describe("with OpenAI integration", () => {
    ...
    it("should use OpenAI when available and return parsed response", async () => {
      ...
      const serviceWithOpenAI =
        new (require("@/lib/ai-service").AIAnalysisService)();
      // Set the openai property directly since we mocked the constructor
      serviceWithOpenAI.openai = new mockOpenAI();

      const result = await serviceWithOpenAI.analyzePosition(
        mockPosition,
        mockMarketData,
        "MODERATE",
      );
      ...
      expect(result.reasoning).toBe(
        "Strong technical indicators suggest taking profits",
      );
    });
