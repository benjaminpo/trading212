...
import { GET, POST, createTrailStopNotification } from '@/app/api/notifications/route'
...
describe('/api/notifications', () => {
  ...
  describe('GET /api/notifications', () => {
    ...
    it('should filter unread notifications only when requested', async () => {
      mockedGetServerSession.mockResolvedValue(mockSession as any)
      mockedPrisma.notification.findMany.mockResolvedValue([mockNotifications[0]])
  ...
  describe('createTrailStopNotification', () => {
    ...
    it('should create trail stop notification for production account', async () => {
      const createdNotification = {
        ...
        type: 'trail_stop_triggered',
        title: '🚨 Trail Stop Triggered - AAPL',
        message: 'Your trail stop order for 10 shares of AAPL has been triggered at $150.00. Please manually execute the sell order in Trading212.',
        data: JSON.stringify({
          symbol: 'AAPL',
          quantity: 10,
          stopPrice: 150.0,
      ...
      const result = await createTrailStopNotification('test-user-id', mockOrderData)
    ...
    it('should create trail stop notification for practice account', async () => {
      ...
      const createdNotification = {
        ...
        type: 'trail_stop_triggered',
        title: '🚨 Trail Stop Triggered - AAPL',
        message: 'Your trail stop order for 10 shares of AAPL has been executed at $150.00.',
        data: JSON.stringify({
          ...practiceOrderData,
          action: 'executed',
        }),
      ...
      const result = await createTrailStopNotification('test-user-id', practiceOrderData)
    ...
    it('should handle database errors', async () => {
      ...
      await expect(createTrailStopNotification('test-user-id', mockOrderData))
