  ...
  describe("GET /api/trading212/optimized/portfolio", () => {
    it("should handle unauthorized requests", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/trading212/optimized/portfolio",
      );
    ...
    it("should handle specific account not found", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/trading212/optimized/portfolio?accountId=nonexistent",
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it("should handle successful portfolio retrieval for default account", async () => {
      ...
      const mockAccountData = {
        portfolio: [{ symbol: "AAPL", quantity: 10, currentPrice: 150 }],
        cacheHit: true,
        lastUpdated: new Date().toISOString(),
      };
      optimizedTrading212Service.canMakeRequest.mockReturnValue(true);
      optimizedTrading212Service.getAccountData.mockResolvedValue(
        mockAccountData,
      );
    ...
    it("should handle successful portfolio retrieval for specific account", async () => {
      ...
      const mockAccountData = {
        portfolio: [{ symbol: "GOOGL", quantity: 5, currentPrice: 100 }],
        cacheHit: false,
        lastUpdated: new Date().toISOString(),
      };
      optimizedTrading212Service.canMakeRequest.mockReturnValue(true);
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/trading212/optimized/portfolio?accountId=acc2",
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it("should handle portfolio retrieval with force refresh", async () => {
      ...
      optimizedTrading212Service.canMakeRequest.mockReturnValue(true);
      optimizedTrading212Service.forceRefreshAccountData.mockResolvedValue(
        mockAccountData,
      );

      const request = new NextRequest(
        "http://localhost:3000/api/trading212/optimized/portfolio?forceRefresh=true",
      );
      const response = await GET(request);
      const data = await response.json();
      ...
      ).toHaveBeenCalledWith("user1", "acc1", "key1", false, false);
    });
    ...
    it("should handle API errors", async () => {
      ...
      optimizedTrading212Service.getAccountData.mockRejectedValue(
        new Error("API Error"),
      );
