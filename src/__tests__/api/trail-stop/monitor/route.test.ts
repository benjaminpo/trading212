...
const mockedTrading212API = Trading212API as jest.MockedClass<
  typeof Trading212API
>;
const mockedCreateTrailStopNotification =
  createTrailStopNotification as jest.MockedFunction<
    typeof createTrailStopNotification
  >;
...
describe("/api/trail-stop/monitor", () => {
  ...
  const mockPositions = [
    {
      ticker: "AAPL",
      currentPrice: 140.0, // Below stop price
      quantity: 10,
    },
    {
      ticker: "GOOGL",
      currentPrice: 2800.0, // Above stop price
      quantity: 5,
    },
  ];
  ...
  beforeEach(() => {
    ...
    mockedTrading212API.mockImplementation(
      (apiKey: string, _isPractice?: boolean) => {
        const mockAPI = {
  ...
  describe("POST /api/trail-stop/monitor", () => {
    it("should process active trail stop orders", async () => {
      ...
      const _request = new NextRequest(
        "http://localhost:3000/api/trail-stop/monitor",
        { method: "POST" },
      );
    ...
    it("should handle no active orders", async () => {
      ...
      const _request2 = new NextRequest(
        "http://localhost:3000/api/trail-stop/monitor",
        { method: "POST" },
      );
      const response = await POST();
    ...
    it("should handle database errors", async () => {
      mockedPrisma.trailStopLossOrder.findMany.mockRejectedValue(
    ...
    it("should handle notification creation errors", async () => {
      ...
      mockedCreateTrailStopNotification.mockRejectedValue(
    ...
    it("should handle order update errors", async () => {
      ...
      mockedPrisma.trailStopLossOrder.update.mockRejectedValue(
      ...
      mockedTrading212API.mockImplementation(
        () =>
          ({
            ...
            getAccount: jest
              .fn()
              .mockResolvedValue({ id: "account-1", currencyCode: "USD" }),
          }) as any,
      );
    ...
    it("should handle positions not found for order symbol", async () => {
      const mockTrading212API = {
        getPositions: jest
          .fn()
          .mockResolvedValue([{ ticker: "GOOGL", currentPrice: 2800.0 }]),
      };
    ...
    it("should handle orders with no positions data", async () => {
      const mockTrading212API = {
        getAccountInfo: jest
          .fn()
          .mockResolvedValue({ account: { currency: "USD", cash: 1000 } }),
    ...
    it("should handle orders with positions but no matching symbol", async () => {
      ...
      const mockTrading212API = {
        getAccountInfo: jest
          ...
          .mockResolvedValue({ account: { currency: "USD", cash: 1000 } }),
        getPositions: jest.fn().mockResolvedValue({
          positions: [{ ticker: "AAPL", quantity: 100, currentPrice: 150 }],
        }),
      };
      mockedTrading212API.mockImplementation(() => mockTrading212API as any);

      mockedPrisma.trailStopLossOrder.findMany.mockResolvedValue([
        orderWithDifferentSymbol,
      ] as any);
