...
describe('/api/trail-stop/orders/[orderId]', () => {
  ...
  describe('PUT', () => {
    ...
    it('updates trail stop order successfully', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/trail-stop/orders/1', {
        method: 'PUT',
        body: JSON.stringify({ trailAmount: 7.50, trailPercent: 5.00 }),
    ...
    it('handles database errors', async () => {
      const { prisma } = require('@/lib/prisma')
      prisma.trailStopLossOrder.findFirst.mockResolvedValue(mockOrder)
      prisma.trailStopLossOrder.update.mockRejectedValue(new Error('Database error'))

      const request = new NextRequest('http://localhost:3000/api/trail-stop/orders/1', {
        method: 'PUT',
        body: JSON.stringify({ trailAmount: 7.50 }),
        headers: { 'Content-Type': 'application/json' },
      })
  ...
  describe('DELETE', () => {
    ...
    it('handles database errors', async () => {
      const { prisma } = require('@/lib/prisma')
      prisma.trailStopLossOrder.findFirst.mockResolvedValue(mockOrder)
      prisma.trailStopLossOrder.delete.mockRejectedValue(new Error('Database error'))
