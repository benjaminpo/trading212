...
describe('DailyPnLChart - Branch Coverage', () => {
  ...
  it('should handle empty daily P/L data', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ dailyPnL: [], summary: { totalDays: 0, totalPnLChange: 0, averageDailyPnL: 0 } }),
    } as Response)

    render(<DailyPnLChart />)
    
    await waitFor(() => {
      expect(screen.getByText('No daily P/L data available yet')).toBeInTheDocument()
    })
  })
  ...
  it('should handle non-Error exception', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Failed to fetch daily P/L data')).toBeInTheDocument()
    })
  })
  ...
  it('should handle capture snapshot button click successfully', async () => {
    ...
    const captureButton = document.querySelector('button svg[class*="calendar"]')?.closest('button')
    if (captureButton) {
  ...
  it('should display summary data correctly', async () => {
    ...
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ dailyPnL: [mockDailyPnLData[0]], summary: summaryWithNulls }),
    } as Response)
  ...
  it('should show refreshing state during operations', async () => {
    ...
    const refreshButton = document.querySelector('button svg[class*="refresh-cw"]')?.closest('button')
    if (refreshButton) {
      fireEvent.click(refreshButton)
    }
    // Skip button interaction if button is not found
