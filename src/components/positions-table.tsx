            ...
            {filteredAndSortedPositions.length > 0 ? (
              filteredAndSortedPositions.map((position, index) => (
                ...
                >
                  ...
                  <td className="py-3 px-2 text-right text-slate-700 dark:text-slate-300 hidden sm:table-cell">
                    {position.quantity || 0}
                  </td>
                  <td className="py-3 px-2 text-right text-slate-700 dark:text-slate-300 hidden md:table-cell">
                    {formatCurrency(position.averagePrice || 0, currency)}
                  </td>
                  <td className="py-3 px-2 text-right text-slate-700 dark:text-slate-300">
                    {formatCurrency(position.currentPrice || 0, currency)}
                  </td>
                  <td className="py-3 px-2 text-right font-medium text-slate-900 dark:text-slate-100 hidden md:table-cell">
                    {formatCurrency(position.marketValue || 0, currency)}
                  </td>
                  <td
                    className={`py-3 px-2 text-right font-medium ${(position.ppl || 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                  >
                    {(position.ppl || 0) >= 0 ? "+" : ""}
                    {formatCurrency(position.ppl || 0, currency)}
                  </td>
                  <td
                    className={`py-3 px-2 text-right font-medium ${(position.pplPercent || 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                  >
                    {(position.pplPercent || 0) >= 0 ? "+" : ""}
              ...
              <tr>
                <td
                  ...
                  className="py-8 text-center text-slate-600 dark:text-slate-400"
                >
                  {searchTerm
