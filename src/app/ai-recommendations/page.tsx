  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          `Analysis completed! Generated ${recommendationsCount} recommendations in ${executionTime}ms (${cacheHits} cache hits)`,
        );
      } else {
              ...
              <div className="flex items-center w-full sm:w-auto">
                <Link href="/dashboard">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="mr-2 sm:mr-4 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors"
                  >
                ...
                <div className="flex-1 min-w-0">
                  <h1 className="text-xl sm:text-3xl font-bold text-gray-900 dark:text-slate-100 flex items-center">
                    <Brain className="h-6 w-6 sm:h-8 sm:w-8 mr-2 sm:mr-3 text-blue-600 dark:text-blue-400 flex-shrink-0" />
                    <span className="truncate">AI Recommendations</span>
                  </h1>
                  <p className="text-sm sm:text-base text-gray-600 dark:text-slate-300 hidden sm:block">
                    AI-powered exit strategy suggestions for your positions
                  </p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 w-full sm:w-auto">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    onClick={runAnalysis}
                    disabled={analyzing}
                    className="bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white transition-all duration-200 shadow-lg hover:shadow-xl flex-1 sm:flex-none"
                  >
                    <RefreshCw
                      className={`h-4 w-4 sm:mr-2 ${analyzing ? "animate-spin" : ""}`}
                    />
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700 dark:text-yellow-300 leading-relaxed">
                    These AI-powered recommendations are for informational
                    purposes only and should not be considered as financial
                    advice. The AI analysis is based on historical data, current
                    market conditions, and position performance, but cannot
                    predict future market movements with certainty. Always
                    conduct your own research, consider your risk tolerance, and
                    consult with a qualified financial advisor before making
                    investment decisions. Past performance does not guarantee
            ...
            <Card className="text-center py-12 bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border-gray-200/50 dark:border-slate-700/50 shadow-xl dark:shadow-slate-900/20">
              <CardContent>
                ...
                <p className="text-gray-600 dark:text-slate-300 mb-6">
                  Run an AI analysis to get personalized exit strategy
                  recommendations for your positions.
                </p>
                <Button
                  onClick={runAnalysis}
                  disabled={analyzing}
            ...
            <div className="space-y-6">
              ...
              <Card className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border-gray-200/50 dark:border-slate-700/50 shadow-xl dark:shadow-slate-900/20">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <div className="text-center p-3 sm:p-4 rounded-lg bg-red-50/80 dark:bg-red-900/20 border border-red-200/50 dark:border-red-800/30">
                      <div className="text-xl sm:text-2xl font-bold text-red-600 dark:text-red-400">
                        {
                          recommendations.filter(
                            (r) => r.recommendationType === "EXIT",
                          ).length
                        }
                      </div>
                    ...
                    <div className="text-center p-3 sm:p-4 rounded-lg bg-blue-50/80 dark:bg-blue-900/20 border border-blue-200/50 dark:border-blue-800/30">
                      <div className="text-xl sm:text-2xl font-bold text-blue-600 dark:text-blue-400">
                        {
                          recommendations.filter(
                            (r) => r.recommendationType === "HOLD",
                          ).length
                        }
                      </div>
                      <div className="text-xs sm:text-sm text-gray-600 dark:text-slate-300">
                    ...
                    <div className="text-center p-3 sm:p-4 rounded-lg bg-orange-50/80 dark:bg-orange-900/20 border border-orange-200/50 dark:border-orange-800/30">
                      <div className="text-xl sm:text-2xl font-bold text-orange-600 dark:text-orange-400">
                        {
                          recommendations.filter(
                            (r) => r.recommendationType === "REDUCE",
                          ).length
                        }
                      </div>
                      <div className="text-xs sm:text-sm text-gray-600 dark:text-slate-300">
                    ...
                    <div className="text-center p-3 sm:p-4 rounded-lg bg-green-50/80 dark:bg-green-900/20 border border-green-200/50 dark:border-green-800/30">
              ...
              <div className="space-y-3 sm:space-y-4">
                {recommendations.map((rec) => (
                  <Card
                    key={rec.id}
                    className={`${getRecommendationColor(rec.recommendationType)} border-l-4 transition-all duration-200 hover:shadow-xl dark:hover:shadow-slate-900/20`}
                  >
                    <CardHeader>
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <CardTitle className="flex items-center text-gray-900 dark:text-slate-100">
                          ...
                          <span className="ml-2 sm:ml-4 text-base sm:text-lg font-bold">
                            {rec.recommendationType}
                          </span>
                        </CardTitle>
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(rec.riskLevel)} w-fit`}
                          >
                            {rec.riskLevel} Risk
                          </span>
                    ...
                    <CardContent className="space-y-3 sm:space-y-4">
                      ...
                      <div className="space-y-3 sm:space-y-0">
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:hidden">
                          <div className="p-3 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg border border-gray-200/50 dark:border-slate-700/50">
                            <div className="text-xs text-gray-600 dark:text-slate-400">
                              Quantity
                            </div>
                            <div className="font-semibold text-gray-900 dark:text-slate-100">
                              {rec.position.quantity}
                            </div>
                          </div>
                          <div className="p-3 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg border border-gray-200/50 dark:border-slate-700/50">
                            <div className="text-xs text-gray-600 dark:text-slate-400">
                            ...
                            <div className="font-semibold text-gray-900 dark:text-slate-100">
                              ${rec.position.averagePrice?.toFixed(2) || "N/A"}
                            </div>
                          </div>
                          <div className="p-3 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg border border-gray-200/50 dark:border-slate-700/50">
                            <div className="text-xs text-gray-600 dark:text-slate-400">
                            ...
                            <div className="font-semibold text-gray-900 dark:text-slate-100">
                              ${rec.position.currentPrice.toFixed(2)}
                            </div>
                          </div>
                          <div className="p-3 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg border border-gray-200/50 dark:border-slate-700/50">
                            <div className="text-xs text-gray-600 dark:text-slate-400">
                            ...
                            <div
                              className={`font-semibold ${rec.position.pnl >= 0 ? "profit" : "loss"}`}
                            >
                        ...
                        {/* Desktop: Grid layout */}
                        <div className="hidden sm:grid grid-cols-2 md:grid-cols-5 gap-4 p-4 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg border border-gray-200/50 dark:border-slate-700/50">
                          <div>
                      ...
                      <div className="p-4 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg border border-gray-200/50 dark:border-slate-700/50">
                        <div className="text-center">
                          ...
                          <div
                            className={`text-2xl font-bold ${rec.position.pnlPercent >= 0 ? "profit" : "loss"}`}
                          >
                            {rec.position.pnlPercent >= 0 ? "+" : ""}
                            {rec.position.pnlPercent.toFixed(2)}%
                          </div>
                        </div>
                      </div>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm text-gray-700 dark:text-gray-300">
                              <span className="font-medium">
                                {rec.accountInfo.name}
                              </span>
                              <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">
                                (
                                {rec.accountInfo.isPractice
                                  ? "Practice"
                                  : "Live"}
                                )
                              </span>
                      ...
                      {/* Feedback */}
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-4 border-t border-gray-200/50 dark:border-slate-700/50">
                        <div className="flex items-center space-x-2">
