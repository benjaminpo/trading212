...
export async function GET(request: NextRequest) {
  try {
    ...
    const multiAccountData =
      await optimizedTrading212Service.getMultiAccountData(
        session.user.id,
        accounts,
        includeOrders,
      );
    ...
    const aggregatedData =
      await optimizedTrading212Service.getAggregatedAccountData(
        session.user.id,
        accounts,
      );
    ...
    const response = {
      accounts: multiAccountData.map((result) => ({
        id: result.accountId,
        name:
          user.trading212Accounts.find((acc) => acc.id === result.accountId)
            ?.name || "Unknown",
        data: result.data,
        error: result.error,
      ...
      connected: multiAccountData.some(
        (result) => result.data && !result.error,
      ),
      cacheStats: {
        totalCacheHits: multiAccountData.filter((result) => result.cacheHit)
          .length,
