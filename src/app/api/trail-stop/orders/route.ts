...
export async function POST(request: NextRequest) {
  try {
    ...
    const { symbol, quantity, trailAmount, trailPercent, isPractice = true, accountId } = await request.json()
    ...
    console.log(`✅ Trail stop order created: ${order.symbol} (${order.quantity} shares) - ${order.isPractice ? 'Practice' : 'Production'} mode`)
