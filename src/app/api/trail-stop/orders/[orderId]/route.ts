...
export async function PUT(
  ...
  try {
    ...
    const { quantity, trailAmount, trailPercent, stopPrice, isActive } = await request.json()
    ...
    if (trailPercent !== undefined && (trailPercent <= 0 || trailPercent >= 100)) {
      return NextResponse.json(
    ...
    const updatedOrder = await prisma.trailStopLossOrder.update({
      ...
      data: {
        ...
        ...(trailAmount !== undefined && { trailAmount: parseFloat(trailAmount) }),
        ...(trailPercent !== undefined && { trailPercent: trailPercent ? parseFloat(trailPercent) : null }),
        ...(stopPrice !== undefined && { stopPrice: stopPrice ? parseFloat(stopPrice) : null }),
        ...(isActive !== undefined && { isActive }),
        updatedAt: new Date()
      }
    })

    console.log(`✅ Trail stop order updated: ${updatedOrder.symbol} - Active: ${updatedOrder.isActive}`)
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ message: 'Trail stop order deleted successfully' })
  } catch (error) {
