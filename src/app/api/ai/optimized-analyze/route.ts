...
export async function POST(request: NextRequest) {
  try {
    ...
    const { analysisType = 'BATCH_ANALYSIS', accountId, forceRefresh = false } = await request.json()

    const startTime = Date.now()
    
    console.log(`🤖 Optimized AI Analysis: ${analysisType} for account ${accountId || 'all'}`)
    ...
    for (const account of accountsToAnalyze) {
      try {
        ...
        console.log(`📊 Collected ${positions.length} positions from account ${account.name}`)
    ...
    console.log(`🎯 Starting batch AI analysis for ${allPositions.length} positions`)
    ...
    const analysisResult = await optimizedAIService.analyzePositionsBatch(batchRequest)
    ...
    for (const [accountId, positions] of accountPositions.entries()) {
      ...
      const accountRecommendations = analysisResult.recommendations.filter((_, index) => {
        const startIndex = allPositions.findIndex(p => p.symbol === positions[0]?.symbol)
      ...
      for (const [index, recommendation] of accountRecommendations.entries()) {
        ...
        try {
          ...
          if (!dbPosition) {
            console.log(`⚠️ Database position not found for ${position.symbol} in account ${account.id}`)
            continue
          }
          ...
          console.error(`❌ Failed to save recommendation for ${position.symbol}:`, error)
        }
      }
    }
    ...
    console.log(`✅ Optimized AI Analysis Complete: ${savedRecommendations.length} recommendations saved, ${analysisResult.cacheHits} cache hits, ${executionTime}ms`)
