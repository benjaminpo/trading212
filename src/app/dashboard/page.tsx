...
export default function Dashboard() {
  ...
  const loadAggregatedAccountData = useCallback(async () => {
    ...
    if (totalInvestedValue > 0) {
      aggregatedStats.totalPnLPercent = (aggregatedStats.totalPnL / totalInvestedValue) * 100
      aggregatedStats.todayPnLPercent = (aggregatedStats.todayPnL / totalInvestedValue) * 100
    }
    ...
    console.log('📊 Aggregated Dashboard Data:', {
      ...
      calculationDetails: {
        ...
        calculatedPnLPercent: totalInvestedValue > 0 ? (aggregatedStats.totalPnL / totalInvestedValue) * 100 : 0
      }
    })
    ...
    setConnectionStatus({
      ...
      accountName: connectedAccounts > 0 ? `${connectedAccounts} Accounts (${accountNames.join(', ')})` : 'No accounts connected',
      mode: 'AGGREGATED',
          ...
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center py-4 lg:py-6 space-y-4 lg:space-y-0">
            ...
            <div className="flex-1">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-2">
                <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-50">Dashboard</h1>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300">
                  ...
                  {connectionStatus.connected && connectionStatus.cash !== undefined && (
                    <span className="block sm:inline sm:ml-2 text-xs sm:text-sm">
                      • Available Cash: {formatCurrency(connectionStatus.cash || 0, connectionStatus.currency || 'USD')}
                    </span>
                  )}
                </p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
          <Card className="hover:shadow-xl hover:shadow-green-500/10 transition-all duration-300 border border-green-200/50 dark:border-green-800/30 shadow-lg bg-gradient-to-br from-white via-green-50/50 to-emerald-50/80 dark:from-slate-800/90 dark:via-green-950/20 dark:to-emerald-950/30 backdrop-blur-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2">
              <CardTitle className="text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200">Total P/L</CardTitle>
            ...
            <CardContent className="pt-2">
              <div className={`text-lg sm:text-3xl font-bold mb-1 ${stats.totalPnL >= 0 ? 'profit' : 'loss'}`}>
                {formatCurrency(stats.totalPnL, stats.currency || 'USD')}
              </div>
              <div className="flex items-center space-x-1 sm:space-x-2">
                <Badge variant={stats.totalPnLPercent >= 0 ? 'success' : 'error'} className="text-xs">
                  {stats.totalPnLPercent >= 0 ? '+' : ''}{stats.totalPnLPercent.toFixed(2)}%
                </Badge>
                <span className="text-xs text-slate-600 dark:text-slate-400 hidden sm:inline">from start</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 border border-blue-200/50 dark:border-blue-800/30 shadow-lg bg-gradient-to-br from-white via-blue-50/50 to-cyan-50/80 dark:from-slate-800/90 dark:via-blue-950/20 dark:to-cyan-950/30 backdrop-blur-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2">
              <CardTitle className="text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200">Today&apos;s P/L</CardTitle>
              <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shadow-lg ${stats.todayPnL >= 0 ? 'bg-gradient-to-br from-green-500 to-emerald-600 shadow-green-500/20' : 'bg-gradient-to-br from-red-500 to-rose-600 shadow-red-500/20'}`}>
                {stats.todayPnL >= 0 ? (
            ...
            <CardContent className="pt-2">
              <div className={`text-lg sm:text-3xl font-bold mb-1 ${stats.todayPnL >= 0 ? 'profit' : 'loss'}`}>
                {formatCurrency(stats.todayPnL, stats.currency || 'USD')}
              </div>
              <div className="flex items-center space-x-1 sm:space-x-2">
                <Badge variant={stats.todayPnLPercent >= 0 ? 'success' : 'error'} className="text-xs">
                  {stats.todayPnLPercent >= 0 ? '+' : ''}{stats.todayPnLPercent.toFixed(2)}%
                </Badge>
                <span className="text-xs text-slate-600 dark:text-slate-400 hidden sm:inline">today</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="hover:shadow-xl hover:shadow-purple-500/10 transition-all duration-300 border border-purple-200/50 dark:border-purple-800/30 shadow-lg bg-gradient-to-br from-white via-purple-50/50 to-violet-50/80 dark:from-slate-800/90 dark:via-purple-950/20 dark:to-violet-950/30 backdrop-blur-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2">
              <CardTitle className="text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200">Active Positions</CardTitle>
            ...
            <CardContent className="pt-2">
              <div className="text-lg sm:text-3xl font-bold mb-1 text-purple-700 dark:text-purple-300">{stats.activePositions}</div>
              <div className="flex items-center space-x-1 sm:space-x-2">
                <Badge variant="secondary" className="text-xs bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300">
                  Active
                </Badge>
                <span className="text-xs text-slate-600 dark:text-slate-400 hidden sm:inline">positions</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="hover:shadow-xl hover:shadow-orange-500/10 transition-all duration-300 border border-orange-200/50 dark:border-orange-800/30 shadow-lg bg-gradient-to-br from-white via-orange-50/50 to-amber-50/80 dark:from-slate-800/90 dark:via-orange-950/20 dark:to-amber-950/30 backdrop-blur-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 sm:pb-2">
              <CardTitle className="text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200">AI Recommendations</CardTitle>
            ...
            <CardContent className="pt-2">
              <div className="text-lg sm:text-3xl font-bold mb-1 text-orange-700 dark:text-orange-300">{stats.aiRecommendations}</div>
              <div className="flex items-center space-x-1 sm:space-x-2">
                <Badge className="text-xs bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300 border-0">
                  <Zap className="w-3 h-3 mr-1" />
                  New
                </Badge>
                <span className="text-xs text-slate-600 dark:text-slate-400 hidden sm:inline">suggestions</span>
              </div>
            </CardContent>
          </Card>
        </div>
          ...
          <Card className="mt-8 border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/20">
            <CardHeader>
              ...
              <CardDescription className="text-blue-700 dark:text-blue-300">
                You have {stats.aiRecommendations} new AI recommendations waiting for review
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <p className="text-blue-800 dark:text-blue-200">
                  Our AI has analyzed your positions and found actionable exit strategies to optimize your portfolio.
                </p>
                <Link href="/ai-recommendations">
