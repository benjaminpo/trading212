...
export class OptimizedAIService {
  ...
  private generateCacheKey(position: PositionData, riskProfile: string): string {
    return `ai_analysis_${position.symbol}_${position.quantity}_${position.currentPrice}_${position.pnlPercent}_${riskProfile}`
  }

  private async getCachedRecommendation(cacheKey: string): Promise<AIRecommendationResult | null> {
    try {
      ...
      if (cached) {
        ...
        return {
          recommendationType: cached.recommendationType as 'HOLD' | 'SELL' | 'BUY_MORE' | 'EXIT',
  ...
  private async analyzePositionBatch(
    ...
    try {
      // Build batch prompt
      const batchPrompt = this.buildBatchAnalysisPrompt(positions, marketData, riskProfile)
...
`
  ...
  private parseBatchAIResponse(response: string, positions: PositionData[]): AIRecommendationResult[] {
    try {
  ...
  async analyzePositionsBatch(request: BatchAnalysisRequest): Promise<BatchAnalysisResult> {
    const startTime = Date.now()
    let cacheHits = 0
    const totalTokens = 0

    console.log(`🎯 Batch AI Analysis: ${request.positions.length} positions for account ${request.accountId}`)
    ...
    console.log(`✅ Batch Analysis Complete: ${allRecommendations.length} recommendations, ${cacheHits} cache hits, ${executionTime}ms`)
  ...
  async clearCache(userId?: string, accountId?: string): Promise<void> {
    try {
      ...
      console.log(`🗑️ AI Cache cleared for user: ${userId}, account: ${accountId}`)
    } catch (error) {
