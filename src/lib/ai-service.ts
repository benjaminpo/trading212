        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert financial advisor specializing in exit strategies for stock positions. Provide detailed, actionable recommendations based on technical and fundamental analysis.",
          },
          {
            role: "user",
            content: prompt,
...
`;
  ...
  private ruleBasedAnalysis(
    ...
    const priceFromHigh =
      ((marketData.high52Week - marketData.price) / marketData.high52Week) *
      100;
    const priceFromLow =
      ((marketData.price - marketData.low52Week) / marketData.low52Week) * 100;

    // Rule-based logic
    let recommendationType: AIRecommendationResult["recommendationType"] =
      "HOLD";
    let confidence = 0.6;
    let reasoning = "";
    let suggestedAction = "";
    ...
    if (pnlPercent > 20 && priceFromHigh < 10) {
      ...
      confidence = 0.8;
      reasoning =
        "Strong gains achieved and price near 52-week high. Consider taking profits.";
      suggestedAction =
        "Sell position to lock in profits. Consider setting trailing stop if holding.";
      riskLevel = "LOW";
    } else if (pnlPercent < -15) {
      recommendationType = "EXIT";
      confidence = 0.7;
      reasoning =
        "Significant losses accumulated. Consider cutting losses to preserve capital.";
      suggestedAction =
        "Sell position to limit further losses. Reassess investment thesis.";
      riskLevel = "HIGH";
    } else if (pnlPercent > 10 && riskProfile === "CONSERVATIVE") {
      recommendationType = "REDUCE";
      confidence = 0.7;
      reasoning =
        "Good profits achieved. Conservative profile suggests taking some profits.";
      suggestedAction =
        "Sell 25-50% of position to reduce risk while maintaining upside exposure.";
      riskLevel = "LOW";
    } else if (pnlPercent < -5 && priceFromLow > 50) {
      recommendationType = "HOLD";
      confidence = 0.6;
      reasoning =
        "Minor losses but price well above 52-week low. Monitor for reversal.";
      suggestedAction =
        "Hold position but set stop-loss at -10%. Monitor for trend changes.";
      riskLevel = "MEDIUM";
    } else {
      recommendationType = "HOLD";
      confidence = 0.5;
      reasoning =
        "Position within normal range. Continue monitoring market conditions.";
      suggestedAction =
        "Maintain current position. Review regularly for changes in fundamentals.";
      riskLevel = "MEDIUM";
    }
    ...
    return {
      ...
      suggestedAction,
      targetPrice:
        recommendationType === "EXIT" ? undefined : marketData.price * 1.1,
  ...
  async analyzeBulkPositions(
    ...
    for (const position of positions) {
      const symbolMarketData = marketData.find(
        (m) => m.symbol === position.symbol,
      );
      if (symbolMarketData) {
        const recommendation = await this.analyzePosition(
          position,
          symbolMarketData,
          riskProfile,
        );
