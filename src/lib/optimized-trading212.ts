import { Trading212Account, Trading212Position, Trading212Order } from './trading212'
...
export class OptimizedTrading212Service {
  ...
  async getAccountData(
    ...
    // Check cache first
    const cached = await apiCache.get<OptimizedAccountData>(userId, accountId, 'account')
    if (cached) {
  ...
  async getPortfolioData(
    ...
    // Check cache first
    const cached = await apiCache.get<OptimizedPortfolioData>(userId, accountId, 'portfolio')
    if (cached) {
  ...
  async getMultiAccountData(
    userId: string,
    accounts: Array<{ id: string; apiKey: string; isPractice: boolean; name: string }>,
    includeOrders: boolean = false
  ): Promise<MultiAccountResult[]> {
    console.log(`🎯 Multi-account optimized fetch for ${accounts.length} accounts`)
  ...
  async getAggregatedAccountData(
    ...
    totalStats.totalPnLPercent = totalStats.totalValue > 0 
      ? (totalStats.totalPnL / (totalStats.totalValue - totalStats.totalPnL)) * 100 
      : 0
  ...
  async invalidateCache(userId: string, accountId?: string, dataType?: 'portfolio' | 'account' | 'orders' | 'positions'): Promise<void> {
    await apiCache.invalidate(userId, accountId, dataType)
    console.log(`🗑️ Cache invalidated for user ${userId}, account ${accountId}, type ${dataType}`)
  }
  ...
  async forceRefreshAccountData(
    ...
    // Fetch fresh data
    return this.getAccountData(userId, accountId, apiKey, isPractice, includeOrders)
  }
  ...
  async backgroundSync(
    ...
    try {
      ...
      console.log(`✅ Background sync completed for ${accountsToSync.length} accounts`)
    } catch (error) {
...
export const optimizedTrading212Service = OptimizedTrading212Service.getInstance()
