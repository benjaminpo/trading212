...
export class APIBatcher {
  ...
  private scheduleBatchExecution(batchKey: string, apiKey: string, isPractice: boolean): void {
  ...
  private async executeBatch(batchKey: string, apiKey: string, isPractice: boolean): Promise<void> {
    ...
    const accountGroups = new Map<string, BatchedRequest[]>()
    ...
    const executionPromises = Array.from(accountGroups.entries()).map(([accountId, accountRequests]) =>
      this.executeAccountBatch(accountId, accountRequests, apiKey, isPractice)
    )
  ...
  private async executeAccountBatch(
    ...
    console.log(`🔄 Executing batch for account ${accountId}: ${requestTypes.join(', ')}`)
    ...
    try {
      ...
      for (const result of successfulResults) {
        ...
        await apiCache.set(userId, accountId, result.type as 'portfolio' | 'account' | 'orders' | 'positions', result.data)
      }

      // Resolve all requests with appropriate data
      requests.forEach(request => {
        const result = successfulResults.find(r => r.type === request.requestType)
        if (result) {
          ...
          request.reject(failedResult && failedResult.status === 'rejected' ? failedResult.reason : new Error('Request failed'))
        }
      })

    } catch (error) {
      console.error(`❌ Batch execution failed for account ${accountId}:`, error)
  ...
  async fetchAccountData(
    ...
    const [accountData, portfolioData, ordersData] = await Promise.allSettled([
      ...
      includeOrders ? this.request(userId, accountId, 'orders', apiKey, isPractice) : Promise.resolve([])
    ])

    const account = accountData.status === 'fulfilled' ? accountData.value : null
    const portfolio = portfolioData.status === 'fulfilled' ? (portfolioData.value as unknown[]) : []
    const orders = ordersData.status === 'fulfilled' ? (ordersData.value as unknown[]) : []
